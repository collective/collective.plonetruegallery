...
def replace_gallery_objects(portal):
    ...
    while len(galleries) > 0:
        ...
        new_gallery = parent[
            parent.invokeFactory("Folder", parent.generateUniqueId())
        ]
        ...
        new_settings.gallery_type = (
            old_gallery.getType() == "default"
            and "basic"
            or old_gallery.getType()
        )
        new_settings.size = old_gallery.getSize()
        new_settings.display_type = (
            old_gallery.getDisplayType() == "classic"
            and "slideshow"
            or old_gallery.getDisplayType()
        )
        ...
        new_settings.picasa_username = getattr(
            old_gallery, 'picasaUsername', None
        )
        new_settings.picasa_album = getattr(old_gallery, 'picasaAlbum', None)
        new_settings.flickr_username = getattr(
            old_gallery, 'flickrUsername', None
        )
...
def check_should_upgrade_to_0_8_1a3(context):
    try:
        return (
            int(
                context.getLastVersionForProfile(
                    default_profile.lstrip('profile-')
                )[0]
            )
            < 7
        )
